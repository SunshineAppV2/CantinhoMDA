...
import { User as UserIcon, Lock, Shield, Pencil, Save, X, Key, Globe, Building2, Search, AlertTriangle } from 'lucide-react';
...
export function EditClubModal({ club, onClose, onSave }: EditClubModalProps) {
    ...
    const handleSearchUser = async () => {
        ...
        try {
            ...
            if (res.data && res.data.length > 0) {
                const user = res.data.find((u: any) => u.email.toLowerCase() === searchEmail.toLowerCase().trim()) || res.data[0];
                setFoundUser(user);
            } else {
    ...
    const handleAssociateDirector = async () => {
        if (!foundUser) return;
        if (!confirm(`Associar ${foundUser.name} como Diretor deste Clube?`)) return;

        try {
            const baseUpdate: any = { role: 'OWNER', clubId: club.id };
            // Se o user já for Owner em outro lugar, backend vai chiar ou remover de lá (depende da logica de update).
            // Nosso users.service não remove ownership antiga automaticamente (só previne multiplos owners NUM MESMO CLUBE, mas user sendo owner de MULTIPLOS clubes não é checado explicitamente lá, mas role é unica).
            // Vamos confiar no backend.

            await api.patch(`/users/${foundUser.id}`, baseUpdate);
            toast.success('Diretor Associado!');
            fetchMembers();
            setFoundUser(null);
            setSearchEmail('');
        } catch (e) {
    ...
    const handleResetClub = async () => {
        if (!confirm('ATENÇÃO: Você tem certeza ABSOLUTA que deseja zerar este clube?')) return;
        if (!confirm('Esta ação NÃO PODE ser desfeita. Todo o histórico financeiro, de reuniões e pontuações será APAGADO. Continuar?')) return;

        setResetting(true);
        try {
            await api.post(`/clubs/${club.id}/reset-data`);
            toast.success('Clube resetado com sucesso! Todo o histórico foi apagado.');
            fetchMembers();
        } catch (error: any) {
            toast.error(error.response?.data?.message || 'Erro ao resetar clube.');
        } finally {
            setResetting(false);
        }
    };
            ...
            <div className="space-y-6 max-h-[80vh] overflow-y-auto pr-2 custom-scrollbar" translate="no">
                ...
                <div className="space-y-4">
                    ...
                    {loadingMembers ? (
                        ...
                        <div className="bg-slate-50 border border-slate-200 border-dashed rounded-lg p-6 space-y-4">
                            <div className="text-sm text-slate-500 italic text-center">Nenhum diretor (Owner) identificado.</div>

                            <div className="space-y-3">
                                <h5 className="text-xs font-bold text-slate-700 uppercase mb-2 text-center">Associar Novo Diretor</h5>
                                <div className="flex gap-2">
                                    <input
                                        ...
                                        className="flex-1 px-3 py-2 border rounded text-sm outline-none focus:ring-2 focus:ring-blue-500"
                                    />
                                    <button
                                        onClick={handleSearchUser}
                                        disabled={searching || !searchEmail}
                                        className="px-4 py-2 bg-slate-200 text-slate-700 text-xs font-bold rounded hover:bg-slate-300 transition-colors"
                                    >
                                ...
                                {foundUser && (
                                    <div className="flex items-center justify-between p-3 bg-white border border-green-200 rounded-lg animate-in fade-in slide-in-from-top-2">
                                        <div>
                                            ...
                                            <div className="text-[10px] text-slate-400 mt-1">Cargo Atual: {foundUser.role}</div>
                                        </div>
                                        <button
                                            onClick={handleAssociateDirector}
